...
def process_state_pmq_meet():
    ...
    print("Поздравляю!Ты нашел себе нового друга,вы нашли много общего и теперь обдумываете план побега")
    score += 20
    stage += 1
...
def process_state_pmq_m_plan1():
    ...
    has_lockpick += 1
    guard_alert += 30
    print(f"Ваши респекты {score}, найдееные вещи (отмычка), подозрение охраны {guard_alert}, теперь вы на {stage} этапе")

    print("1 - дождаться ночи и сбежать из камеры а после и из тюрьмы")
    print("2 - попробавать получить работу на кухне чтобы сбежать через ту дверь куда приносят продукты")
    print("3 - подкупить охранника с помощью денег знакомых с наружи чтоб вас ночью не запирали и помогли пройти по коридорам")

    change = int(input("Выбери один из вариантов, сможешь ли ты сделать правильный выбор?    "))
...
def process_state_pmq_m_plan2():
    ...
    print("Мда, стащить то ложку получилось, а вот донести ее до камеры нет, вас обыскали ее нашел охранник, теперь вы отправляетесь в карцер на месяц") 
    score += 20
    stage += 1
    lives -= 1
    print(f"Вы проиграли, ваш текущий результат: респекты {score}, уровень {stage}, жизни {lives} ") 
...
def process_state_pmq_m_plan3():
    ...
    print("У тебя получилось стащить ключ карту, но твой сокамерник забрал ее себе и здал тебя. теперь ты оправляешься в карцер на год ")
    score += 30
    stage += 1
    lives -= 1
    print(f"Вы проиграли, ваш текущий результат: респекты {score}, уровень {stage}, жизни {lives}")
...
def process_state_pmq_m_p1_nigh():
    ...
    print("Вы дошли до запасного выхода через медпункт, но ваша отмычка сломалась засчет чего вы создали кучу шума. Вы и ваш сокамерник отправляетесь в карцер на год.")
    ...
    print(f"Ваш текущий результат: респекты {score}, найденные предметы (отмычка) {has_lockpick}, жизни {lives}")
...
def process_state_pmq_m_p1_work():
    ...
    print("Вам удалось уговорить бабу Галю с помощью своей харизмы и вы получили работу. Во время привоза продуктов вы вышли под предлогом покурить и спрятались в грузовике и когда они доехали до следующей точки вы сбежали.")
    score += 50
    stage += 1

    print(f"Ваш текущий результат: респекты {score}, найденные предметы (отмычка) {has_lockpick}, жизни {lives}")

    print("ПОЗДРРАВЛЯЕМ ВЫ СБЕЖАЛИ! Предлагаем сыграть снова и попытаться найти новую концовку")
...
def process_state_pmq_m_p1_money():
    ...
    print("Вы смогли подкупить подкупить охраника и вас не запирали в комнате, но когда вы шли по коридорам вашего подкупленного охраника не было и вас поймали и отправили в карцер на год")
    score += 20
    stage += 1
    lives -= 1
...
def check_lives():
    ...
    if lives == 0:
        print("У вас закончились жизни.Вы отправляетесь на смертную казнь. Ну ничего сбежите в следующей жизни")
        dies += 1
        lives += 3
...
def process_state_pmq_search():
    ...
    print("Еба ты сыщик, ты нашел отмычку, наверно она осталась после прошлого сокамерника. Но не все так гладко как ты думаешь, ты сильно ярко среагировал на находку тебя теперь подозревает сокамерник и охранник")    
    score += 15
    stage += 1
    has_lockpick += 1
    guard_alert += 40
    print(f"Ваши респекты: {score}, найденых вспомогательных штук (отмычек) {has_lockpick}, подозрение охраны на {guard_alert},теперь вы на {score} этапе игры")
...
def process_state_pmq_loose_lockpic():
    ...
    print("Твой сокамерник наехал на тебя при охраннике и у тебя отобрали отмычку. Ты отправляешься в карцер на месяц")
    score += 5
    stage += 1
    lives -= 1
    print(f"Вы проиграли, ваш текущий результат: респекты {score}, уровень {stage}, жизни {lives}")
...
def process_pmq_ratted():
    ...
    print("Пока вы ночью вскрывали дверь ваш сокамерник проснулся, а после утром здал вас. Вы отправляетесь в карцер на месяц")
    score += 10
    stage += 1
    lives -= 1
    print(f"Вы проиграли, ваш текущий результат: респекты {score}, уровень {stage}, жизни {lives}")
...
def process_pmq_make_friends():
    ...
    print("Когда вы рассказали своему сокамернику про отмычку он сказал что он со своей бандой давно готовил план побега и им как раз не хватало отмычки")
    score += 35
    stage += 1
...
def process_state_pmq_mk_good():
    ...
    stage += 1
    has_lockpick -=1
    print(f"Вы выиграли! Ваш текущий результат: респекты {score}, найденные предметы (отмычка){has_lockpick}, уровень {stage}")
...
def process_state_pmq_mk_bad():
    ...
    print("Вас избили на дневной прогулке и отобрали отмычку. После вас подставили и сздали охране")
    score += 5
    stage += 1
    has_lockpick -= 1
    lives -= 1
    print(f"Вы проиграли, ваш текущий результат: респекты {score}, уровень {stage}, найденные предметы (отмычка) {has_lockpick}  жизни {lives}")
...
def process_state_pmq_seat():
    ...
    print("Ты с сокамерником теперь в нейтральных отношениях, охранник заметил твое послушание и предложил тебе работу на кухне")
    score += 40
    stage += 1
    print(f"Ваши респекты {score}, теперь вы на {stage} этапе игры")
...
def process_state_pmq_s_gala():
    ...
    print("ОМГ! Баба Галя оказалась вашей фанаткой и у вас все взаимно теперь вы с ней ")
    score += 40
    stage += 1
...
def process_state_pmq_s_quit_work():
    ...
    print("Другие сокамерники узнали о том что тебе дали работу на кухне,за что они тебя возненавидели. После на прогулке состоялась маштабная драка где ты не выжил")
    score += 15
    stage += 1
    lives -= 1 

    print(f"Ваши респекты: {score}, вы проиграли на {stage} этапе игры, ваши жизни {lives}")
...
def process_state_pmq_s_more_food():
    ...
    print("Доверие охранников к вам выросло, но такое поведение не понравилось вашим сокамерникам, поэтому на утренней прогулке вы попали в передрягу. Охранники заступились за вас, они наснесли раны холостыми патронами, но вам серавно досталось, вас доставили в больницу, но не успели спасти")
...
def process_state_sg_get_key():
    ...
    print("О НЕТ! Пока баба Галя искала для вас ключ она влюбилась в охранника и рассказала ему про то что вы хотите сбежать. Теперь вы и без ключа и без Гали так вас еще и в карцер на месяц отправили...") 
    score += 5
    stage += 1
    lives -= 1
    print(f"Ваши респекты: {score}, вы проиграли на {stage} этапе игры, ваши жизни {lives}")
...
def process_state_sg_cook():
    ...
    print("О НЕТ! Вы начали самостоятельно готовить и в итоге кухня загорелась, пока вы пытались все потушить вы надышались угарным газом и баба Галя вынесла вас из здания на руках. ОТКРЫТА СЕКРЕТНАЯ КОНЦОВКА! ") 
    score += 1000
    stage += 1
...
def process_state_sg_quit_cooking():
    ...
    print("Бывалым тюремщикам не понравилось то что вам дали такую работу и во время прогулки вас избили, а потом на вас настучали что вы якобы будите использовать инвентарь с кухни для побега. Из за этого вы отправляетесь в карцер на месяц") 
    score += 5
    stage += 1
    lives -= 1
    print(f"Ваши респекты: {score}, вы проиграли на {stage} этапе игры, ваши жизни {lives}")
