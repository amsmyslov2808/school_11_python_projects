...
def input_int(message: str, min_val: int, max_val: int) -> int:
    ...
    message — приглашение ко вводу, а min_val и max_val — допустимые границы.
    Обе границы включены: например, диапазон 0..2 принимает 0, 1 и 2.
    """
...
def input_float(message: str, min_val: float, max_val: float) -> float:
    ...
    Возвращаемое значение имеет тип float и обязано попадать в закрытый диапазон
    от min_val до max_val. Дробная часть вводится через точку, например ``4.7``.
    """
    is_correct_input = False
    input_int = 0
...
def input_str(message: str, min_len: int, max_len: int) -> str:
    """Запрашивает текст, длина которого входит в заданный диапазон.

    Проверяется только количество символов. Функция не удаляет пробелы и не
    меняет регистр, поэтому возвращает строку именно в том виде, как её ввели.
    """
    is_correct_input = False
    input_str = ""
...
def input_date(message: str, min_date: date, max_date: date) -> date:
    ...
    Результат — объект date. Благодаря этому вызывающий код может сравнивать
    даты хронологически, не разбирая день, месяц и год вручную.
    """
    is_correct_input = False
