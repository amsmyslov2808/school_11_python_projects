...
# Сейчас номера сортировки по ID совпадают с номерами сортировки по дате.
# Поэтому при значении 5 или 6 сработает первая подходящая ветка if/elif, а ветки
# сортировки по ID ниже не будут достигнуты. Это особенность текущей версии кода.
SORT_BY_ID_ASC = 5
SORT_BY_ID_DESC = 6
...
# Значение поиска по цене совпадает со значением поиска по части названия.
# Пока функция реализует только SEARCH_BY_PART_NAME, остальные режимы — заготовки.
SEARCH_BY_PRICE = 1
...
def set_start_product_id(product_id: int):
    ...
    # Без global присваивание создало бы новую локальную переменную внутри функции.
    global global_product_id
    # Следующий вызов get_next_product_id() вернёт product_id + 1.
...
def sort_products_by_type_sort(products: list[Product], type_sort: int):
    ...
    if type_sort == SORT_BY_PRICE_ASC:
        ...
        # По смыслу эта ветка сортирует даты по убыванию, однако в условии сейчас
        # повторена константа ASC. Из-за предыдущего elif ветка фактически недоступна.
        offset = 0
        temp = None
        is_sort = False
        ...
        while is_sort == False:
            ...
            for i in range(0, len(products) - 1 - offset):
                # Более поздние даты должны были бы перемещаться к началу списка.
                if products[i + 1].release_date > products[i].release_date:
                    temp = products[i]
                    products[i] = products[i + 1]
                    products[i + 1] = temp
        ...
        # Сортировка по ID от меньшего номера к большему. При текущих значениях
        # констант эта ветка перекрыта расположенными выше проверками даты.
        offset = 0
        temp = None
        is_sort = False
        ...
        # Сортировка по ID от большего номера к меньшему; ветка также перекрыта.
        offset = 0
        temp = None
        is_sort = False
...
def find_products_by_type_search(
    ...
    В текущей версии готов только поиск по части названия. Поиск не изменяет
    исходный список: совпавшие объекты складываются в новый список.
    """
    ...
    # Для ещё не реализованных режимов функция доходит до конца и неявно
    # возвращает None, хотя аннотация типа обещает list[Product].
...
def buy_product(products: list[Product], search_id: int, request_amount: int) -> bool:
    ...
    # Заглушка оставляет место для проверки ID, остатка на складе и уменьшения amount.
    pass
            ...
            for _ in range(count_products):
                ...
                products.append(
                    Product(
                        ...
                        # Текстовую дату разбираем по тому же формату, в котором
                        # release_date_to_str() сохраняет её в файл.
...
def save_products_to_txt_file(products: list[Product], filename: str) -> bool:
    ...
    строку. Такой файл рассчитан прежде всего на чтение функцией
    load_products_from_txt_file(), а не человеком.
    """
    try:
