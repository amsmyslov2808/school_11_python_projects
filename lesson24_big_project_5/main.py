# Это точка входа в консольное приложение магазина. Файл создаёт начальный каталог,
# показывает меню и передаёт обработку товаров функциям из соседних модулей.

# Подключаем класс-карточку Product, тип даты и готовые функции из соседних файлов.
from product import Product
from datetime import date
# Звёздочка импортирует все общедоступные имена модуля. Поэтому ниже можно вызывать
# input_int(), get_product_by_id() и другие функции без префикса имени модуля.
...
mock_products = [
    ...
    # Именованные аргументы позволяют явно видеть, в какое поле попадёт значение.
    Product(
...
# В список добавляются ссылки на те же объекты Product, новые копии не создаются.
...
# Ниже оставлен пример ручной проверки печати и поиска. Символ # превращает строки
# в комментарии, поэтому при обычном запуске этот демонстрационный код не работает.
# print_products()
# print("\n\n\n")

# finded_products = find_products_by_type_search(products, SEARCH_BY_PART_NAME, "поМАда")
...
def work_with_buyer_menu():
    ...
    while is_run_buyer_menu == True:
        ...
        # В текущей версии действия покупателя ещё не реализованы. pass является
        # допустимой пустой командой и позволяет заранее оставить структуру меню.
        if choose_action_buyer_menu == 1:
            # Здесь планируется поиск одной карточки по её ID.
            pass
        elif choose_action_buyer_menu == 2:
            # Здесь планируется выбор поля и направления сортировки.
            pass
        elif choose_action_buyer_menu == 3:
            # Здесь планируется поиск по названию, категории или цене.
            pass
        elif choose_action_buyer_menu == 4:
            # Здесь планируется покупка и уменьшение остатка товара.
            pass
        elif choose_action_buyer_menu == 5:
            # Здесь планируется создание человекочитаемого файла с таблицей.
            pass
        elif choose_action_buyer_menu == 0:
            is_run_buyer_menu = False

        # Несколько переводов строк и длинная линия визуально отделяют операции.
        print("\n\n")
        print("=" * 125)
...
def work_with_administrator_menu():
    # Функция загрузки заменяет весь список новым, поэтому здесь требуется доступ
    # именно к глобальной переменной products, а не создание локальной переменной.
    global products
...
# Переменная находится вне функции, поэтому этот код запускается сразу при старте main.py.
is_run = True
